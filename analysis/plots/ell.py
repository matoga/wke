"""Coherence-length figure: ℓ = (f(k→0)/n)^(1/3), so that ℓ³ = V in equilibrium.
...
two steps) are drawn with bars. Older runs fall back to the occupation snapshots: the same fit
for f₀, dℓ²/dt by finite differences in t, no bars, the t = 0 snapshot dropped; the title says so.
"""
import numpy as np
import matplotlib.pyplot as plt
from common import Style, GUIDE, LABEL, point_kw, legends, save, run_id
...
def series(r):
    ...
    if p.get('ellRate'):
        ...
        return ell * kxi, y, e, ell * kxi / x, False
    sn = r.get('snapshots', {})
    if len(sn.get('n_k', [])) < 3:
        return None
    ...
    ell = np.cbrt(np.array(f0) / s['density_um3'])
...
def make(study_dir, study, runs):
    ...
    for r in runs:
        ...
        if d is None:
            continue
        x, y, e, lk, snap = d
        keep = np.isfinite(y) & np.isfinite(e) & np.isfinite(lk)
    ...
    for r, x, y, e, lk, _ in data:
        rid = run_id(r)
        arrays[f'{rid}__ell_kxi'] = x; arrays[f'{rid}__ell_rate'] = y
        arrays[f'{rid}__ell_rate_err'] = e; arrays[f'{rid}__ell_kp'] = lk
    ...
    for j, ax in enumerate(axes[:2]):
        ...
        ax.set_ylabel(r'$(m/\hbar)\,\mathrm{d}\ell^{2}/\mathrm{d}t$')
    ...
    ax.set_ylabel(r'$\ell\,k_p$')
    for a in axes:
        a.set_xlabel(r'$\ell\,k_\xi$'); a.grid(alpha=.3, which='both')
    ...
    fig.suptitle(f"ℓ = (f(k→0)/n)^(1/3); {', '.join(models)}; {', '.join(acc)} accuracy; {src}")
