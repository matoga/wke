...
n_k/n (µm³) against k (µm⁻¹), log-log, so that the k→0 plateau reads ℓ³; k_p of the panel marked.
...
def at_kp(r, kp):
    """(k, n_k/n) at smooth peak k_p, or None when k_p ≥ k_ξ or outside the snapshots."""
    sn = r.get('snapshots', {})
    ...
    return k, f / r['settings']['density_um3']
...
def make(study_dir, study, runs):
    ...
    for ax, kp in zip(axes, KP_LEVELS):
        ...
        for r in runs:
            ...
            handles.append(h)
            arrays[f'{run_id(r)}__kp{kp:g}__k_um_inv'] = k; arrays[f'{run_id(r)}__kp{kp:g}__nk_over_n_um3'] = y
    ...
    fig.suptitle(f"n_k at fixed k_p (runs with k_p < k_ξ only); {', '.join(models)}; {', '.join(acc)} accuracy; "
                 'interpolated between snapshots')
    fig.tight_layout(rect=(0, 0, 1 - 2.4 / fig.get_figwidth(), 1))
    x0 = 1 - 2.3 / fig.get_figwidth()
    hs = [Line2D([], [], color='0.25', ls=dash[s], lw=1.2, label=f'{s}, E/N = {style.EN[s]:.0f} nK') for s in style.states]
